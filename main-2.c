...
#define TAM_SOLUCAO 10000 /* MUDAR AQUI QNT DE NOS LIDOS */
...
void lerDistanciasArquivo(char *nome, Cidades *cidades)
{
...
void vizinho_mais_proximo(Cidades *cidades, int *conjuntoSolucao)
{
    ...
    while (i < TAM_SOLUCAO - 1)
    {
        ...
        for (int j = 0; j < TAM_SOLUCAO; j++)
        {
            ...
            printf("dist para no: %d -> %d | ", j+1, vetorDistancias[converteIndice(noAtual,j)]);
            printf("vis?: %d \n", cidades[j].visitado);
            */

            if(distAux > vetorDistancias[converteIndice(noAtual,j)] && cidades[j].visitado == 0)
            {
                distAux = vetorDistancias[converteIndice(noAtual,j)];
                vizMaisProx = j; 
            }
        }
    ...
    distTotal += vetorDistancias[converteIndice(noAtual,noInicial)]; // matrizDistancias[noAtual][noInicial];
